...
**Task** Run the following code to make a copy of the Grantee field called grantee_clean, then use the `key_collision_merge` function from refinr to attempt to standardize similar values. Store it as a new object called cleaned_md_grant_loans Click the black arrow on the table to examine the Grantee and grantee_clean columns. Looking at the results on the first 3 pages only (first 30 rows), name two changes the refinr package made, by comparing Grantee to grantee_clean. **Answer** It merged the names that were similar and changed them to one common name. It also changed names that were lowercase to uppercase. It probably noticed one that was all uppercase and decided to change the format.
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** It grouped the similar names so we don't have duplicates, the different versions of names are combined with other choices, and there are significantly less results from 17,765 to 376.
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with W. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Answer** I'm not sure if I'm seeing something wrong but there isn't a big difference between the two datasets. All the numbers seem to align along with the formatting of the garantees. 
