...
**Answer** It's important since you don't want your new data to get mixed up with the old stuff. 
...
**Answer:** Under the expenditure date column, it now says <date> instead of <chr> so it recognizes it as a date now instead of random letters. 
...
**Answer** I think it's organizing the dates by year, month, day, but it only gives the first day of the month not specific days where they spent a lot of money. 
...
**Answer** I think the code is recognizing the math that needs to be done, but doesn't know what sort of percent we want it in so it doesn't move the decimal place accordingly. And it orders it from the least to the greatest. 
...
**Answer** It ordered it from greatest to least where the county that had the highest percentage of votes for Moore comes first in the data and it tells you which county it was. This changed because in the arrange code we added "desc" so it could be arranged that way.
...
**Answer** I guess it is a little bit interesting since we see which county did he have the least percentage of votes working your way to the most percentage of votes. 
...
**Answer** "str_to_lower" does the opposite of upper and makes everything lowercase. "str_to_title" makes the first letter of each word in a title uppercase. 
...
**Answer** It's recognizing the different states and abbreviating them so we easily recognize the state without having to look at the entire address. 
...
**Answer** Yes, it does make sense to me. The code is asking to separate by state abbreviation then add up each amount of money that is being spent to show much each state is contributing, correct?
...
1. Which payee contributed the most money?

2. What day did Maryland receive the most money?

3. I would like to ask the same question from last week: Which department offered the most classes in Fall 2023?
