...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **Answer** There are only eight columns within this data. The object ID, zip codes, zip name, the shape length and area, and geometry. 
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** Everything shows up like a regular data set, but the geometry column gives you a "multipolygon" description.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It gives us a full map of Maryland with each city mapped out divided by its zip code. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This codeblock just loads the tigris library which allows us to run a function like "counties."
...
**Task** Run the following code. Describe the output in the space below. **Answer** It gives us a bunch of identifying systems for all the counties in the U.S. like GEOID, fips codes and name. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It gives us all the counties, including Baltimore city into one data frame. If we only 
...
**Task** Run the following code. Describe the output in the space below. **Answer** On this one, the bay is closed up unlike the other map we created. The county lines go into the bay area. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The zip code boundaries end at the waters edge. The zip code boundaries define where the land is, while the county boundaries include the water in it's boundaries.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This section loads the data in, cleans up the coloumn names, saves it to a new data frame then only selects two section, zip and the data from March of this year.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It joins the two data frames together so it shows the number of foreclosures each zip code has. Although all of them don't have notices associated with them, we still want to see them on the map since they exist. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It shows us which zip codes have a large to small amount of foreclosure notices with the lighter color depicting the high number and the dark color depicting the low number. 
...
**Task** Run the following code. Describe the output in the space below. Be sure to input your census api key. **Answer** Since I already loaded the other libraries, all I had to run this time was the tidycensus so I could collect the census data when needed. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The first function cuts off the rest of the md_county_notices and flips it so it has more rows and less columns. Despite having the md_county_population census, we don't have a geometry column so we can't make a map. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It adds the geometry column. This way we can create a map. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We would have to clean up the data because there isn't anything to clearly join together. The name column has county and state while the county column has just the county name. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It has fixed the data frame and made everything uniform so that when we combine the data frames they will easily go together. It also only selected the columns we wanted to see.
```{r}
...
**Task** Run the following code. Describe the output in the space below. **Answer** Since we fixed the previous data frame, everything combined easily with the population, geometry, notices and popoulation included. If we were to keep it the same way before, we would have problems making sure R knows what we're trying to do. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This data frame shows us the rate of notices per county per 1,000 people. It shows us the population, how many notices they've gotten, and where it's located. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It now gives us each county color coded with the county names on top of each.
...
**Task** Run the following code. Describe the output in the space below. **Answer** This gives us a different color pallete so it's easier for us to distinguish between what counties fall on the highest or lower end. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** This map has more detailed numbers and shows a better correlation between each county. It also gives a wider range of percentages compared to the previous map.
