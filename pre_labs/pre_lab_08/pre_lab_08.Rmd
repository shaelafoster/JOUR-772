...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer** First, we're creating a new dataframe and putting the info we're looking for on it. Then, we're adding a month column that has each name spelled out and not using numbers to identify the months. From there, we do math, grouping by month, counting, then arranging from greatest to least.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** There's nothing, just blank space. 
...
#ggplot(data=baltco_911_calls)
```

Next we need to tell ggplot what kind of chart to make.

In ggplot, we work with two key concepts called geometries(what kind) (abbreviated frequently as geom) and aesthetics (abbreviated as aes).
...
Aesthetics (where it's going) help ggplot know what component of our data to visualize -- why we'll visualize values from one column instead of another.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We get a bar chart that displays the code we did above. It shows the month on the x-axis and count on the y-axis. The weight code shows how thick the bars will be.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It has reordered the chart to see which month has the least calls and gradually shift into the most calls. The only problem is, the months aren't in order anymore.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It shifts the chart from vertical to horizontal, flipping the axes. It also shows the data from greatest to least unlike what we did in the above code. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** It removes the grey background, the bars are thinner and there's more space in between the months on the y-axis. Everything looks clean and simple.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** It adds a blue background to the chart, adds a line between the bottom of the chart and the word "count" on the x-axis. But it also puts the months and y-axis title close together, which is confusing to understand. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** A title for the chart has been added, the title for the y-axis is changed to "total calls" and there is an attribution in the lower right corner. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It shows every call that occurred on each day from Feb. 2022 - Feb. 2023.
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It shows the calls that happened each day with the points indicating when it occurred. For me, it looks like sound waves. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** This code added day by day breaks. The only problem is, the dates aren't clear on the y-axis. They are close together and not legible. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** It puts the dates on an angle, so we can somewhat see what the dates are now, although it isn't perfect it's better than what it was before. 
