...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** The columns where the districts are listed fall under "Maryland," but everything else doesn't have a name. Also, there are an excessive amount of periods after each jurisdictions name. 
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** The problems that remain are the names for the columns are numbers and the periods after each jurisdiction are still there. 
