...
**Task** Export the CSV file to your pre_lab_05 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** The coloumns where the districts are listed fall under "Maryland," but everything else doesn't have a name. Also, there are an excessive amount of periods after the jurisdiction names. 
...
**Task** Load the data without headers by running the codeblock below. What problems remain? **Answer** The problems that remain are the names for the columns and the periods after each jursidiction are still there. 
...
**Task** Run the codeblock below. What else do we need to do? **Answer** After fixing the column names, the periods are the jurisdictions are still in the way.
