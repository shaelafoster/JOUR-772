...
**Task** Run the following code in the gray-colored codeblock below to extract all tables in the HTML and save it as a data frame. What do you notice about the columns? **Answer** The date column is a character and not a date, it's also now an actual data frame that we can open and is saved in our environment. 
...
**Task** Run the following code to extract the table we want, clean up the names and change the date column to an actual date. Look at the dataframe; what other information about these releases might you want to add? **Answer** We need the press releases from earlier in the year. 
...
tags there are in the HTML source? **Answer** It suggests that there are multiple lists but three <ul> tags that I can see on my view. We have a lot of <il> tags for everything including social media. 
...
tags. Describe the output: **Answer** It gives us all the text that's inside of the <li> tags. 
...
**Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **Answer** It gives us a table with all the text that's inside of the <li> tags with a column header of "value."
