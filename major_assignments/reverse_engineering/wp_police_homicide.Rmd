...
40-25
```
Answer: We can prove that there has been a 15% change of the arrest rate in Baltimore following the death of Freddie Gray. 


##Statement 5: For most of the decade before 2015, Baltimore’s annual homicide arrest rate hovered at about 40 percent. 

Step 1: Isolate data for only Baltimore looking at 2007-2014 and calculate arrest rates for each year.

2007:
```{r}
baltimore|>
  filter(reported_year == '2007') |>
...
2007 arrest rate: 41.78%


2008:
```{r}
baltimore |>
  filter(reported_year == '2008') |>
  summarise(count_homicides = n())
```
...
2008 arrest rate: 40.59%


2009:
```{r}
baltimore |>
  filter(reported_year == '2009')|>
  summarise(count_homicides = n())
```
...
(77/197)*100
```
2011 arrest rate: 39.08%


2012:
```{r}
baltimore |>
  filter(reported_year == '2012') |>
  summarise(count_homicides = n())
```
...
```
2012 arrest rate: 39.81%


2013:
```{r}
baltimore |>
  filter(reported_year == '2013')|>
  summarise(count_homicides = n())
```
...
(92/231)*100
```
2013 arrest rate: 39.82%


Based on previous data, we already have a data frame saved with the arrest rate in 2014. Below is the math verifying it. 
```{r}
(86/211)*100
```
2014 arrest rate: 40.75%



Step 2: Now we can take these percentages and figure out the average arrest rate from 2007 to 2014. 
```{r}
(41.78+40.59+44.06+42.27+39.08+39.81+39.82+40.75)/8
```

The average arrest rate is 41.02%. 

Answer: After completing the data, we ca confirm that the average arrest rate for the decade before 2015 was hovering 41% in the city of Baltimore. 



Overall: From completing the data analysis, we can confirm the Washington Post correctly used and detailed data throughout their article. 
