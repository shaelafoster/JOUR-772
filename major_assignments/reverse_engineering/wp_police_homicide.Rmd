...
baltimore
```
### Note: Attempted to chhange the dates to actual dates, but when I filter the data, it doesn't pick up what we're looking for. How can we fix this? Because of this we kept it in the same format as the second milestone. ###
...
Homicide arrests made in major cities in 2014:
```{r} 
homicide_data |>
  filter(disposition == "Closed by arrest") |>
  filter(between(reported_date, 20140101, 20141231)) |>
  group_by(city) |>
  summarise(
    count=n()
  ) |>
...
Homicide arrests made in major cities in 2017:

```{r} 
homicide_data |>
  filter(disposition == "Closed by arrest") |>
  filter(between(reported_date, 20170101, 20171231)) |>
  group_by(city) |>
  summarise(
    count=n()
  ) |>
...
```{r}
#Here we are calculating the number of homicides in major cities in 2014 that were open and had no arrest
homicide_data |> 
...
```{r}
#Here we are calculating the number of homicides in major cities in 2014 that were closed and had no arrest
homicide_data |> 
