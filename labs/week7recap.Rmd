...
**A1:** This dataset contains data about earthquakes that have happened in the last month with times, date, depth, etc. 
...
**A2:** There are 9,774 records in this dataset. The records indicate how many earthquakes happen each day, what time it was, the exact location, magnitude, etc. The useful information we can gather is trying to see how deep it was, how strong and how many times do earthquakes happen everyday.
...
**A3:** The depth is 669.9820 and it's magnitude is 4.20.
...
**A4:** There are only 13 earthquakes who's magnitude is over 6.00.
...
```{r}
earthquakes |>
  filter(mag > 6.00 & depth < 20)
```

**A5:** There are oly six that fit both of these criteria.
...
**A6:** There are about 7,446 earthquakes that fit this criteria. 
...
**A7:** There were 3,446 earthquakes that took place in Alaska.
...
**A8:** The two most common tremors are earthquake and quarry blast.
...
**A9:** The average depth for earthquakes are 25.24.
...
**A10:** By using the mutate and hour function to just extract the specific hours.
...
serious <- earthquakes |>
  mutate(
    serious = case_when(mag > 3 ~ "serious",
    .default = "not serious"
  ) )
  
```

**A11:** By using the mutate function with case_when to make a new coloumn and make the ones that are greater than 3 to say "serious" and nothing that fit that criteria to say "not serious."
...
**A12**: 11:00am has the most serious earthquakes with 447 while 2:00pm has fewer serious earthquakes with 360. To be honest, everytime is the most dangerous time. 

------------------------------------------------------------------------

#### **Q13** What's another question you are interested in and how would you ask it in R?
### Which location has experienced the most earthquake type?

**A13**: 7km NW of The Geysers, CA has the most earthquake type with 177. 
```{r}
earthquakes|>
